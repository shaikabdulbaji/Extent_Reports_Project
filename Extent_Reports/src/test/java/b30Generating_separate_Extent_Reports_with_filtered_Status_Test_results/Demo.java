...
public class Demo {
	...
	public static void main(String[] args) throws IOException {
		...
		sparkReportpass.filter().statusFilter().as(new Status[] {Status.PASS}).apply();
		reports.attachReporter(sparkReportpass);
		
		File filefail = new File(System.getProperty("user.dir")+"\\ExtentReports\\eReportFail.html");
		ExtentSparkReporter sparkReportfail = new ExtentSparkReporter(filefail);
		sparkReportfail.filter().statusFilter().as(new Status[] {Status.FAIL}).apply();
		reports.attachReporter(sparkReportfail);
		
		File fileskip = new File(System.getProperty("user.dir")+"\\ExtentReports\\eReportSkip.html");
		ExtentSparkReporter sparkReportskip = new ExtentSparkReporter(fileskip);
		sparkReportskip.filter().statusFilter().as(new Status[] {Status.SKIP, Status.WARNING}).apply();
		reports.attachReporter(sparkReportskip);
