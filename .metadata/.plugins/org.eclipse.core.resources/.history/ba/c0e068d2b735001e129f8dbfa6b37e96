package b25Configuration_using_Java_Report_theme_naming_title_time_stamp_format_CSS_style_and_JS_Code;
...
public class Demo {
	...
	public static void main(String[] args) {
		...
		//Changing Extent Report Theme
		sparkReport.config().setTheme(Theme.DARK);
		
		//Changing Extent Report Name
		sparkReport.config().setReportName("Tutorials Ninja Test Execution Results");
		
		//Changing Extent Report Title Name
		sparkReport.config().setDocumentTitle("Tutorials Ninja Test Execution");
		
		//Changing Extent Report Time Stamp
		sparkReport.config().setTimeStampFormat("DD-MM-YYYY hh:mm:ss");
		
		//Changing Extent Report CSS Style
		sparkReport.config().setCss(".badge-primary{background-color:#eac869}");
		
		//Changing Extent Report JS (Remove)
